...
class ApiController extends Controller
{
    ...
    public function initialize(){
        ...
        //部分方法不需要token
        if (!in_array($this->request->params['action'], ['getToken'])){
            if (empty($this->request->query('_token'))){
                exit(json_encode(['code'=>'00006','msg'=>$this->_getMsg('00006')]));
            }elseif(!$this->_checkToken($this->request->query('_token'))){
    ...
    public function getToken()
    {
        ...
        if ($this->_data && is_array($this->_data)){
            ...
            if (isset($this->_data['app'])&&isset($this->_data['key'])){
                ...
                if (!in_array($this->_data['app'], $apps)){
                    $code = '10010';
